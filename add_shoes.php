<?php
    ...
    $target_dir = "productimages/shoes/";
    ...
    if (file_exists($target_file)) {
        ...
        $target=basename($target_file, ".jpg");  
            if($product==$target){
            $sql="INSERT INTO footwear(ShoeSize, Color, Sole_Material, Brand, Price, ShopID, Quantity, PName, ShoeType) VALUES (:size, :color, :material, :brand, :price, :shopid, :quantity, :product, :type);";
...
<head>
    <title>Shoppers Paradigm Add Shoes</title>
    <meta charset="UTF-8">
...
<body style="margin: 0;" id="back2">
    ...
    <div id="formback">
        <h2 style="color: white;">Shoes <br> ShopID:
        <?php
            $shopid = $_SESSION['shopid'];
            echo $shopid; 
            if($error){
                ?>
                <div class="errorWrap" style="color: white;"><strong>ERROR</strong>: <?php echo htmlentities($error); ?> </div>
                <?php 
            } 
            else if($msg){
                ?><div class="succWrap" style="color: white;"><strong>SUCCESS</strong>: <?php echo htmlentities($msg); ?> </div><?php 
            }
        ?>
        </h2>
        <form style="background-color: rgb(61,75,62); border-radius: 10px;padding-top: 10px;" action="#" method="POST" enctype="multipart/form-data">
            ...
            <input id="size" name="size" type="text" placeholder="Enter Shoe size" class="input_field" required><br>
            <input id="color" name="color" type="text" placeholder="Enter Shoe Color" class="input_field" required><br>
            <input id="material" name="material" type="text" placeholder="Enter Sole Matrial" class="input_field" required><br>
            <input id="quantity" name="quantity" type="number" placeholder="Enter Quantity" class="input_field" required><br>
            <input id="type" name="type" type="text" placeholder="Enter Shoe Type" class="input_field" required><br>
